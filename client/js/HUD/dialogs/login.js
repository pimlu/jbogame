define(['../div'],function(div) {
  ...
  login.play=function(all) {
    ...
    +'<table style="margin:0.7em 0"><tr>'
    +'<td>[[login.username]]:</td><td><input type="text" id="name"/></td>'
    +'</tr><tr>'
    +'<td>[[login.password]]:</td><td><input type="password" id="pass"/></td>'
    +'</tr></table>'
    +'<button id="login">[[login.login]]</button> '
    +'<button id="guest">[[login.guest]]</button>'
    +'<x-t n="nbsp" id="feedback" class="logfb"></x-t>'
    ,'dlogin');
    ...
    d.children('#login').click(function() {
      ...
      var qs={name:name,pass:d.find('#pass').val()};
